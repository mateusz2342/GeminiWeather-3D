...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState([
    { sender: 'bot', text: 'Cześć! Jestem Twoim asystentem pogodowym. Zapytaj mnie o temperaturę, deszcz, UV, wiatr czy pogodę na wybraną godzinę!' }
  ]);
  ...
  const handleChatSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      let targetHour = null;
      for (let h = 0; h < 24; h++) {
        if (q.includes(`${h}:00`) || q.includes(` ${h} `) || q.includes(` o ${h}`) || q.includes(`o ${h}`)) {
          targetHour = h;
          break;
        }
      }
      ...
      const hourObj = targetHour !== null && selectedDay?.hourly ? selectedDay.hourly[targetHour] : null;

      if (q.includes('uv')) {
        const val = hourObj ? hourObj.uv : selectedDay?.uvMax;
        reply = `Wskaźnik UV wynosi ${val !== undefined ? val : '--'}. ${val > 5 ? 'Pamiętaj o kremie z filtrem!' : 'Poziom UV jest bezpieczny.'}`;
        ...
        if (isRain) reply = `Tak, ${hourObj ? `o ${targetHour}:00` : 'w tym dniu'} spodziewany jest deszcz. Zabierz parasol!`;
        else if (isSnow) reply = `Spodziewane są opady śniegu.`;
        else reply = `Brak opadów deszczu ${hourObj ? `o ${targetHour}:00` : 'w wybranym czasie'}.`;
      } else if (q.includes('temp') || q.includes('stopni') || q.includes('ciepło') || q.includes('zimno') || q.includes('pogoda')) {
        if (hourObj) {
          reply = `${targetDayIdx === 1 ? 'Jutro' : 'Dzisiaj'} o ${targetHour}:00 będzie około ${hourObj.temp}°C.`;
        } else if (targetDayIdx === 1) {
          reply = `Jutro temperatura wyniesie od ${selectedDay?.minTemp}°C do ${selectedDay?.maxTemp}°C.`;
        } else {
        ...
        reply = `Prędkość wiatru wynosi około ${w || '--'} km/h.`;
          ...
          <>
            ...
            <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
              ...
              <form onSubmit={handleChatSubmit} className="flex gap-2">
                <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Zadaj pytanie (np. 'Jaka temperatura jutro o 18?')..." className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-100" />

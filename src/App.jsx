      ...
      <main className="w-full max-w-md flex-1 flex flex-col gap-4 pb-6">
        ...
        {!loading && !error && weatherData && (
          <>
            ...
            {/* Siatka parametrów: UV, Ciśnienie, Jakość powietrza, Wiatr */}
            ...
            {/* SEKCJA: GODZINOWA DLA WYBRANEGO DNIA */}
            <div className="bg-slate-900/60 border border-slate-800/80 p-4 rounded-2xl">
              <h3 className="text-sm font-semibold text-slate-200 mb-3 flex items-center gap-2">
            ...
            {/* RADAR POGODOWY (MAPA NA ŻYWO) */}
            <div className="bg-slate-900/60 border border-slate-800/80 p-4 rounded-2xl">
              <h3 className="text-sm font-semibold text-slate-200 mb-2 flex items-center gap-2">
                <span>📡</span> Radar Pogodowy (Opady na żywo)
              </h3>
              <p className="text-xs text-slate-400 mb-3">Interaktywna mapa chmur, wiatru i opadów deszczu.</p>
              <div className="w-full h-72 rounded-xl overflow-hidden border border-slate-800 bg-slate-950">
                <iframe 
                  title="Radar pogodowy"
                  src={`https://embed.windy.com/embed2.net/?lat=${weatherData.lat}&lon=${weatherData.lon}&zoom=7&level=surface&overlay=radar&product=radar&menu=&message=&marker=&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=km%2Fh&metricTemp=%C2%B0C&radarRange=-1`}
